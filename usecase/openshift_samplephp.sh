...
SAMPLE_APP_URL=http://samplephp-monitor.myopenshift-cloud.com
...
function createApp {
	...
	echo o $STEP
	$CREATE_APP
	if [ $? -ne 0 ]; then echo $(errorMsg $STEP) ; `exit 1000`; fi
}
...
function curlApp {
	...
	if [ $CURL_CHECK -ne 2 ]; then echo $(errorMsg $STEP) ; `exit 1010`; fi
}
...
function sshApp {
	...
	if [ $SSH_CHECK -ne 1 ];  then echo $(errorMsg $STEP) ; `exit 1020`; fi
}
...
function tailApp {
	...
	if [ ! "$TAIL_CHECK" -ge 2 ];  then echo $(errorMsg $STEP) ; `exit 1030`; fi
}
...
function pushApp {
	...
	if [ $? -ne 0 ]; then echo $(errorMsg $STEP) ; popd > /dev/null ; `exit 1040`; else popd > /dev/null ; fi
}
...
function curlPush {
	...
	PHPINFO_HASH="This program makes use of the Zend Scripting Language Engine"
        ...
        if [ $CURL_CHECK -ne 1 ]; then echo $(errorMsg $STEP) ; `exit 1050`; fi
}
...
function deleteApp {
	...
	echo o $STEP
	$DELETE_APP
	if [ $? -ne 0 ]; then echo $(errorMsg $STEP) ; `exit 1100`; fi
}
